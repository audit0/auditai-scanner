...
-- "Allow authenticated users to read documents": checks the bucket only, so every signed-in user
-- can read every user's files, through the app or straight through the Storage API.
