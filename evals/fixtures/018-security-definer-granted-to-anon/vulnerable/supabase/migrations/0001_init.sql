...
-- The table policies are correct; the dashboard totals come from a SECURITY DEFINER function.
...
-- Dashboard totals. SECURITY DEFINER skips the invoice policies, the tenant comes from the caller's
-- argument, and EXECUTE is granted to anon: anyone with the public anon key reads any tenant's totals.
...
grant execute on function public.tenant_invoice_totals(uuid) to anon, authenticated;
