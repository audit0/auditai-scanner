...
-- "Is the caller a member of this account?" The membership row must be the caller's own.
...
as $$
  select exists (
    select 1 from public.profiles where tenant_id = p_account_id and id = auth.uid()
  )
$$;
