...
/**
 * Authenticates a public-API request; throws when the key is missing, unknown or revoked. The row
 * ids it returns come from the credential lookup, never from the request: the caller cannot choose
 * which key row (or tenant) they stand for.
 */
